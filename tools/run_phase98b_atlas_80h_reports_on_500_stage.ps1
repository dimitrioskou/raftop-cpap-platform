...
# Phase 98B - ATLAS / 80h / Reports Verification on 500-row Stage
# Reads imported 500-stage production data and verifies operational outputs.
...
$ReportPath = Join-Path $ReportsDir ("phase98b_atlas_80h_reports_on_500_stage_" + $Timestamp + ".md")

$SnapshotCsv = Join-Path $ReportsDir ("phase98b_500_stage_operational_snapshot_" + $Timestamp + ".csv")
$AtlasQueueCsv = Join-Path $ReportsDir ("phase98b_500_stage_atlas_priority_queue_" + $Timestamp + ".csv")
$ComplianceSummaryCsv = Join-Path $ReportsDir ("phase98b_500_stage_80h_compliance_summary_" + $Timestamp + ".csv")
$ManagementSnapshotCsv = Join-Path $ReportsDir ("phase98b_500_stage_management_snapshot_" + $Timestamp + ".csv")
$VerificationDoc = Join-Path $DocsDir "98B_ATLAS_80H_REPORTS_ON_500_STAGE.md"
...
Set-Content -Path $ReportPath -Value "# RAFTOP CPAP CARE Pro - Phase 98B ATLAS 80h Reports on 500-row Stage" -Encoding UTF8
Add-Content -Path $ReportPath -Value "" -Encoding UTF8
Add-Content -Path $ReportPath -Value ("Generated: " + (Get-Date -Format "yyyy-MM-dd HH:mm:ss")) -Encoding UTF8
Add-Content -Path $ReportPath -Value "" -Encoding UTF8
Add-Content -Path $ReportPath -Value "Purpose: verify ATLAS, 80h compliance, and management reports using 500-row stage production data." -Encoding UTF8
Add-Content -Path $ReportPath -Value "DATABASE_URL is never printed." -Encoding UTF8
Add-Content -Path $ReportPath -Value "" -Encoding UTF8

Write-Host ""
Write-Host "Running RAFTOP Phase 98B - ATLAS / 80h / Reports Verification on 500-row Stage..."
Write-Host ""

Check-ReportStatus "Phase 98 500-row controlled stage latest status" "phase98_500_row_controlled_stage_apply_*.md" @(
    "PHASE98_500_ROW_CONTROLLED_STAGE_APPLY_READY",
    "PHASE98_500_ROW_CONTROLLED_STAGE_APPLY_READY_WITH_WARNINGS"
)
...
"@
        ...
        if ($SnapshotExit -eq 0) {
            ...
            Add-Result "500-stage operational snapshot exported" "PASS" $SnapshotCsv
        } else {
            Add-Result "500-stage operational snapshot exported" "FAIL" ($SnapshotOutput | Out-String)
        }
    } catch {
        Add-Result "500-stage operational snapshot exported" "FAIL" ("Exception: " + $_.Exception.Message)
    }
}
...
try {
    if (Test-Path $SnapshotCsv) {
        ...
        Add-Result "500-stage operational snapshot readable" "PASS" ("Rows: " + @($Rows).Count)
    }
} catch {
    Add-Result "500-stage operational snapshot readable" "FAIL" ("Could not read snapshot CSV: " + $_.Exception.Message)
}

if (@($Rows).Count -ge 500) {
    Add-Result "500-stage snapshot has 500 rows" "PASS" ("Rows: " + @($Rows).Count)
} else {
    Add-Result "500-stage snapshot has 500 rows" "FAIL" ("Rows: " + @($Rows).Count)
}
...
if (Test-Path $AtlasQueueCsv) {
    Add-Result "500-stage ATLAS priority queue CSV created" "PASS" $AtlasQueueCsv
} else {
    Add-Result "500-stage ATLAS priority queue CSV created" "FAIL" $AtlasQueueCsv
}

if (@($AtlasQueue).Count -gt 0) {
    Add-Result "500-stage ATLAS queue has action rows" "PASS" ("Rows: " + @($AtlasQueue).Count)
} else {
    Add-Result "500-stage ATLAS queue has action rows" "WARN" "No action rows. Stage data may be too clean."
}

if ($Below80h -gt 0) {
    Add-Result "500-stage below 80h patients detected" "PASS" ("Rows: " + $Below80h)
} else {
    Add-Result "500-stage below 80h patients detected" "WARN" "No below-80h rows detected."
}

if ($Compliant80h -gt 0) {
    Add-Result "500-stage 80h compliant patients detected" "PASS" ("Rows: " + $Compliant80h)
} else {
    Add-Result "500-stage 80h compliant patients detected" "WARN" "No compliant rows detected."
}

if ($HighAhi -gt 0) {
    Add-Result "500-stage high AHI patients detected" "PASS" ("Rows: " + $HighAhi)
} else {
    Add-Result "500-stage high AHI patients detected" "WARN" "No high AHI rows detected."
}

if ($HighLeak -gt 0) {
    Add-Result "500-stage high leak patients detected" "PASS" ("Rows: " + $HighLeak)
} else {
    Add-Result "500-stage high leak patients detected" "WARN" "No high leak rows detected."
}
...
if (Test-Path $ComplianceSummaryCsv) {
    Add-Result "500-stage 80h compliance summary CSV created" "PASS" $ComplianceSummaryCsv
} else {
    Add-Result "500-stage 80h compliance summary CSV created" "FAIL" $ComplianceSummaryCsv
}
...
$ManagementSnapshot = @(
    ...
    [PSCustomObject]@{ section = "readiness"; metric = "ready_for_2000_row_stage"; value = "yes_if_buyer_signoff" }
)
...
if (Test-Path $ManagementSnapshotCsv) {
    Add-Result "500-stage management snapshot CSV created" "PASS" $ManagementSnapshotCsv
} else {
    Add-Result "500-stage management snapshot CSV created" "FAIL" $ManagementSnapshotCsv
}

$VerificationDocContent = @"
# RAFTOP CPAP CARE Pro - ATLAS / 80h / Reports Verification on 500-row Stage

REQUIRED_MARKER: PHASE98B_ATLAS_80H_REPORTS_ON_500_STAGE
REQUIRED_MARKER: FIVE_HUNDRED_STAGE_VERIFIED
REQUIRED_MARKER: ATLAS_QUEUE_GENERATED
REQUIRED_MARKER: EIGHTY_HOURS_COMPLIANCE_VERIFIED
REQUIRED_MARKER: READY_FOR_PHASE99_2000_ROW_STAGE_AFTER_SIGNOFF

## Meaning

The 500-row stage data was used to generate operational verification outputs.

## Outputs

500-stage operational snapshot:
$SnapshotCsv

500-stage ATLAS priority queue:
$AtlasQueueCsv

500-stage 80h compliance summary:
$ComplianceSummaryCsv

500-stage management snapshot:
...
Phase 99:
2000-row controlled stage apply after 500-row stage verification.
"@

Set-Content -Path $VerificationDoc -Value $VerificationDocContent -Encoding UTF8

if (Test-Path $VerificationDoc) {
    Add-Result "Phase 98B verification document created" "PASS" $VerificationDoc
} else {
    Add-Result "Phase 98B verification document created" "FAIL" $VerificationDoc
}

foreach ($Marker in @(
    "PHASE98B_ATLAS_80H_REPORTS_ON_500_STAGE",
    "FIVE_HUNDRED_STAGE_VERIFIED",
    "ATLAS_QUEUE_GENERATED",
    "EIGHTY_HOURS_COMPLIANCE_VERIFIED",
    "READY_FOR_PHASE99_2000_ROW_STAGE_AFTER_SIGNOFF"
)) {
...
if ($script:FailCount -gt 0) {
    $FinalStatus = "PHASE98B_ATLAS_80H_REPORTS_ON_500_STAGE_FAILED"
    $ExitCode = 1
} elseif ($script:WarnCount -gt 0) {
    $FinalStatus = "PHASE98B_ATLAS_80H_REPORTS_ON_500_STAGE_READY_WITH_WARNINGS"
    $ExitCode = 0
} else {
    $FinalStatus = "PHASE98B_ATLAS_80H_REPORTS_ON_500_STAGE_READY"
    $ExitCode = 0
}
...
Write-Host ""
Write-Host "============================================================"
Write-Host "RAFTOP CPAP CARE Pro - Phase 98B ATLAS / 80h / Reports on 500-row Stage"
Write-Host "============================================================"
Write-Host ""
