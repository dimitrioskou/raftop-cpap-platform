...
# Phase 99 - 2000-row Controlled Stage Apply
# Imports first 2000 rows from approved CSV or synthetic CSV into production DB.
...
param(
    [string]$CsvPath = "",
    [int]$LimitRows = 2000,
    [string]$ImportStage = "stage_2000_controlled",
...
$ReportPath = Join-Path $ReportsDir ("phase99_2000_row_controlled_stage_apply_" + $Timestamp + ".md")
$StageDoc = Join-Path $DocsDir "99_2000_ROW_CONTROLLED_STAGE_APPLY.md"
$StageRowsCsv = Join-Path $ReportsDir ("phase99_2000_row_stage_rows_" + $Timestamp + ".csv")
$ApplySqlFile = Join-Path $ReportsDir ("phase99_2000_row_stage_apply_" + $Timestamp + "_DO_NOT_SEND.sql")
$VerificationCsv = Join-Path $ReportsDir ("phase99_2000_row_stage_verification_" + $Timestamp + ".csv")
...
Set-Content -Path $ReportPath -Value "# RAFTOP CPAP CARE Pro - Phase 99 2000-row Controlled Stage Apply" -Encoding UTF8
Add-Content -Path $ReportPath -Value "" -Encoding UTF8
Add-Content -Path $ReportPath -Value ("Generated: " + (Get-Date -Format "yyyy-MM-dd HH:mm:ss")) -Encoding UTF8
Add-Content -Path $ReportPath -Value "" -Encoding UTF8
Add-Content -Path $ReportPath -Value "Purpose: controlled 2000-row stage import into production operational tables." -Encoding UTF8
Add-Content -Path $ReportPath -Value "DATABASE_URL is never printed." -Encoding UTF8
Add-Content -Path $ReportPath -Value "" -Encoding UTF8

Write-Host ""
Write-Host "Running RAFTOP Phase 99 - 2000-row Controlled Stage Apply..."
Write-Host ""

Check-ReportStatus "Phase 98B ATLAS 80h 500-stage verification latest status" "phase98b_atlas_80h_reports_on_500_stage_*.md" @(
    "PHASE98B_ATLAS_80H_REPORTS_ON_500_STAGE_READY",
    "PHASE98B_ATLAS_80H_REPORTS_ON_500_STAGE_READY_WITH_WARNINGS"
)
...
if ($LimitRows -ne 2000) {
    Add-Result "Stage row limit is 2000" "WARN" ("LimitRows is " + $LimitRows + ". Expected 2000 for Phase 99.")
} else {
    Add-Result "Stage row limit is 2000" "PASS" "LimitRows = 2000."
}

$StageRows = @($Rows | Select-Object -First $LimitRows)

if (@($StageRows).Count -eq 2000) {
    Add-Result "Stage row count selected" "PASS" "Rows selected: 2000."
} else {
    ...
    try {
        ...
        foreach ($Table in $RequiredTables) {
            ...
            if ($Exit -eq 0 -and (($Out | Out-String).Trim()) -eq "1") {
                Add-Result ("Production table exists before 2000 import: " + $Table) "PASS" "Table exists."
            } else {
                Add-Result ("Production table exists before 2000 import: " + $Table) "FAIL" "Table missing."
            }
        }
    } catch {
...
$SqlLines = @()
$SqlLines += "-- RAFTOP CPAP CARE Pro - Phase 99 2000-row controlled stage apply"
...
$SqlLines += "VALUES ('raftopoulos-production', '$(SqlEscape $ImportBatchId)', '$(SqlEscape $ImportStage)', '$(SqlEscape (Split-Path $CsvPath -Leaf))', $(@($StageRows).Count), 'started', 'Phase 99 controlled 2000-row stage import', 'phase99_script', now());"
$SqlLines += ""
...
$SqlLines += "SET status = 'completed', notes = 'Phase 99 controlled 2000-row stage import completed'"
...
if (Test-Path $ApplySqlFile) {
    Add-Result "2000-row stage SQL generated" "PASS" $ApplySqlFile
} else {
    Add-Result "2000-row stage SQL generated" "FAIL" $ApplySqlFile
}
...
if ($script:FailCount -eq 0) {
    try {
        ...
        if ($ApplyExit -eq 0) {
            Add-Result "2000-row stage SQL applied" "PASS" "psql exit code 0."
        } else {
            Add-Result "2000-row stage SQL applied" "FAIL" ("psql exit code: " + $ApplyExit)
        }
    } catch {
        Add-Result "2000-row stage SQL applied" "FAIL" ("Exception: " + $_.Exception.Message)
    }
} else {
    Add-Result "2000-row stage SQL applied" "FAIL" "Skipped because pre-apply gate has FAIL."
}
...
"@
        ...
        if ($VerifyExit -eq 0) {
            ...
            Add-Result "2000-row stage verification CSV exported" "PASS" $VerificationCsv
        } else {
            Add-Result "2000-row stage verification CSV exported" "FAIL" ($VerifyOutput | Out-String)
        }
        ...
        if ($StagePatientCountExit -eq 0) {
            ...
            if ($CountText -match "^[0-9]+$" -and [int]$CountText -ge 2000) {
                Add-Result "2000 stage patients present after import" "PASS" ("Count: " + $CountText)
            } else {
                Add-Result "2000 stage patients present after import" "FAIL" ("Count: " + $CountText)
            }
        } else {
            Add-Result "2000 stage patients present after import" "FAIL" ($StagePatientCountOutput | Out-String)
        }

    } catch {
        Add-Result "2000-row stage verification" "FAIL" ("Exception: " + $_.Exception.Message)
    }
}

$StageDocContent = @'
# RAFTOP CPAP CARE Pro - 2000-row Controlled Stage Apply

REQUIRED_MARKER: PHASE99_2000_ROW_CONTROLLED_STAGE_APPLY
REQUIRED_MARKER: TWO_THOUSAND_ROW_STAGE_APPLIED
REQUIRED_MARKER: NO_DIRECT_IDENTIFIERS
REQUIRED_MARKER: NO_7000_IMPORT_YET
REQUIRED_MARKER: READY_FOR_PHASE99B_2000_ROW_VERIFICATION

## Meaning

The controlled production stage was expanded from 500 rows to 2000 rows.
...
Phase 99B:
ATLAS / 80h / Reports verification on 2000-row stage.
'@

Set-Content -Path $StageDoc -Value $StageDocContent -Encoding UTF8

if (Test-Path $StageDoc) {
    Add-Result "Phase 99 stage document created" "PASS" $StageDoc
} else {
    Add-Result "Phase 99 stage document created" "FAIL" $StageDoc
}

foreach ($Marker in @(
    "PHASE99_2000_ROW_CONTROLLED_STAGE_APPLY",
    "TWO_THOUSAND_ROW_STAGE_APPLIED",
    "NO_DIRECT_IDENTIFIERS",
    "NO_7000_IMPORT_YET",
    "READY_FOR_PHASE99B_2000_ROW_VERIFICATION"
)) {
...
if ($script:FailCount -gt 0) {
    $FinalStatus = "PHASE99_2000_ROW_CONTROLLED_STAGE_APPLY_FAILED"
    $ExitCode = 1
} elseif ($script:WarnCount -gt 0) {
    $FinalStatus = "PHASE99_2000_ROW_CONTROLLED_STAGE_APPLY_READY_WITH_WARNINGS"
    $ExitCode = 0
} else {
    $FinalStatus = "PHASE99_2000_ROW_CONTROLLED_STAGE_APPLY_READY"
    $ExitCode = 0
}
...
Write-Host ""
Write-Host "============================================================"
Write-Host "RAFTOP CPAP CARE Pro - Phase 99 2000-row Controlled Stage Apply"
Write-Host "============================================================"
Write-Host ""
Write-Host "CSV path:"
Write-Host $CsvPath
