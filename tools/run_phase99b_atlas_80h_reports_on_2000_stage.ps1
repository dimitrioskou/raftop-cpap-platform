...
# Phase 99B - ATLAS / 80h / Reports Verification on 2000-row Stage
# Reads imported 2000-stage production data and verifies operational outputs.
...
$ReportPath = Join-Path $ReportsDir ("phase99b_atlas_80h_reports_on_2000_stage_" + $Timestamp + ".md")

$SnapshotCsv = Join-Path $ReportsDir ("phase98b_2000_stage_operational_snapshot_" + $Timestamp + ".csv")
$AtlasQueueCsv = Join-Path $ReportsDir ("phase98b_2000_stage_atlas_priority_queue_" + $Timestamp + ".csv")
$ComplianceSummaryCsv = Join-Path $ReportsDir ("phase98b_2000_stage_80h_compliance_summary_" + $Timestamp + ".csv")
$ManagementSnapshotCsv = Join-Path $ReportsDir ("phase98b_2000_stage_management_snapshot_" + $Timestamp + ".csv")
$VerificationDoc = Join-Path $DocsDir "99B_ATLAS_80H_REPORTS_ON_2000_STAGE.md"
...
Set-Content -Path $ReportPath -Value "# RAFTOP CPAP CARE Pro - Phase 99B ATLAS 80h Reports on 2000-row Stage" -Encoding UTF8
Add-Content -Path $ReportPath -Value "" -Encoding UTF8
Add-Content -Path $ReportPath -Value ("Generated: " + (Get-Date -Format "yyyy-MM-dd HH:mm:ss")) -Encoding UTF8
Add-Content -Path $ReportPath -Value "" -Encoding UTF8
Add-Content -Path $ReportPath -Value "Purpose: verify ATLAS, 80h compliance, and management reports using 2000-row stage production data." -Encoding UTF8
Add-Content -Path $ReportPath -Value "DATABASE_URL is never printed." -Encoding UTF8
Add-Content -Path $ReportPath -Value "" -Encoding UTF8

Write-Host ""
Write-Host "Running RAFTOP Phase 99B - ATLAS / 80h / Reports Verification on 2000-row Stage..."
Write-Host ""

Check-ReportStatus "Phase 98 500-row controlled stage latest status" "phase99_2000_row_controlled_stage_apply_*.md" @(
    "PHASE99_2000_ROW_CONTROLLED_STAGE_APPLY_READY",
    "PHASE99_2000_ROW_CONTROLLED_STAGE_APPLY_READY_WITH_WARNINGS"
)
...
"@
        ...
        if ($SnapshotExit -eq 0) {
            ...
            Add-Result "2000-stage operational snapshot exported" "PASS" $SnapshotCsv
        } else {
            Add-Result "2000-stage operational snapshot exported" "FAIL" ($SnapshotOutput | Out-String)
        }
    } catch {
        Add-Result "2000-stage operational snapshot exported" "FAIL" ("Exception: " + $_.Exception.Message)
    }
}
...
try {
    if (Test-Path $SnapshotCsv) {
        ...
        Add-Result "2000-stage operational snapshot readable" "PASS" ("Rows: " + @($Rows).Count)
    }
} catch {
    Add-Result "2000-stage operational snapshot readable" "FAIL" ("Could not read snapshot CSV: " + $_.Exception.Message)
}

if (@($Rows).Count -ge 2000) {
    Add-Result "2000-stage snapshot has 2000 rows" "PASS" ("Rows: " + @($Rows).Count)
} else {
    Add-Result "2000-stage snapshot has 2000 rows" "FAIL" ("Rows: " + @($Rows).Count)
}
...
if (Test-Path $AtlasQueueCsv) {
    Add-Result "2000-stage ATLAS priority queue CSV created" "PASS" $AtlasQueueCsv
} else {
    Add-Result "2000-stage ATLAS priority queue CSV created" "FAIL" $AtlasQueueCsv
}

if (@($AtlasQueue).Count -gt 0) {
    Add-Result "2000-stage ATLAS queue has action rows" "PASS" ("Rows: " + @($AtlasQueue).Count)
} else {
    Add-Result "2000-stage ATLAS queue has action rows" "WARN" "No action rows. Stage data may be too clean."
}

if ($Below80h -gt 0) {
    Add-Result "2000-stage below 80h patients detected" "PASS" ("Rows: " + $Below80h)
} else {
    Add-Result "2000-stage below 80h patients detected" "WARN" "No below-80h rows detected."
}

if ($Compliant80h -gt 0) {
    Add-Result "2000-stage 80h compliant patients detected" "PASS" ("Rows: " + $Compliant80h)
} else {
    Add-Result "2000-stage 80h compliant patients detected" "WARN" "No compliant rows detected."
}

if ($HighAhi -gt 0) {
    Add-Result "2000-stage high AHI patients detected" "PASS" ("Rows: " + $HighAhi)
} else {
    Add-Result "2000-stage high AHI patients detected" "WARN" "No high AHI rows detected."
}

if ($HighLeak -gt 0) {
    Add-Result "2000-stage high leak patients detected" "PASS" ("Rows: " + $HighLeak)
} else {
    Add-Result "2000-stage high leak patients detected" "WARN" "No high leak rows detected."
}
...
if (Test-Path $ComplianceSummaryCsv) {
    Add-Result "2000-stage 80h compliance summary CSV created" "PASS" $ComplianceSummaryCsv
} else {
    Add-Result "2000-stage 80h compliance summary CSV created" "FAIL" $ComplianceSummaryCsv
}
...
$ManagementSnapshot = @(
    ...
    [PSCustomObject]@{ section = "readiness"; metric = "ready_for_7000_row_stage"; value = "yes_if_buyer_signoff" }
)
...
if (Test-Path $ManagementSnapshotCsv) {
    Add-Result "2000-stage management snapshot CSV created" "PASS" $ManagementSnapshotCsv
} else {
    Add-Result "2000-stage management snapshot CSV created" "FAIL" $ManagementSnapshotCsv
}

$VerificationDocContent = @"
# RAFTOP CPAP CARE Pro - ATLAS / 80h / Reports Verification on 2000-row Stage

REQUIRED_MARKER: PHASE99B_ATLAS_80H_REPORTS_ON_2000_STAGE
REQUIRED_MARKER: TWO_THOUSAND_STAGE_VERIFIED
REQUIRED_MARKER: ATLAS_QUEUE_GENERATED
REQUIRED_MARKER: EIGHTY_HOURS_COMPLIANCE_VERIFIED
REQUIRED_MARKER: READY_FOR_PHASE100_7000_ROW_STAGE_AFTER_SIGNOFF

## Meaning

The 2000-row stage data was used to generate operational verification outputs.
...
Phase 99:
2000-row controlled stage apply after 2000-row stage verification.
"@

Set-Content -Path $VerificationDoc -Value $VerificationDocContent -Encoding UTF8

if (Test-Path $VerificationDoc) {
    Add-Result "Phase 99B verification document created" "PASS" $VerificationDoc
} else {
    Add-Result "Phase 99B verification document created" "FAIL" $VerificationDoc
}

foreach ($Marker in @(
    "PHASE99B_ATLAS_80H_REPORTS_ON_2000_STAGE",
    "TWO_THOUSAND_STAGE_VERIFIED",
    "ATLAS_QUEUE_GENERATED",
    "EIGHTY_HOURS_COMPLIANCE_VERIFIED",
    "READY_FOR_PHASE100_7000_ROW_STAGE_AFTER_SIGNOFF"
)) {
...
if ($script:FailCount -gt 0) {
    $FinalStatus = "PHASE99B_ATLAS_80H_REPORTS_ON_2000_STAGE_FAILED"
    $ExitCode = 1
} elseif ($script:WarnCount -gt 0) {
    $FinalStatus = "PHASE99B_ATLAS_80H_REPORTS_ON_2000_STAGE_READY_WITH_WARNINGS"
    $ExitCode = 0
} else {
    $FinalStatus = "PHASE99B_ATLAS_80H_REPORTS_ON_2000_STAGE_READY"
    $ExitCode = 0
}
...
Write-Host ""
Write-Host "============================================================"
Write-Host "RAFTOP CPAP CARE Pro - Phase 99B ATLAS / 80h / Reports on 2000-row Stage"
Write-Host "============================================================"
Write-Host ""
