...
# Phase 97 - ATLAS / 80h / Reports Verification on Imported Pilot Data
# Reads imported pilot production data and verifies operational outputs.
...
$ReportPath = Join-Path $ReportsDir ("phase97_atlas_80h_reports_on_pilot_data_" + $Timestamp + ".md")

$SnapshotCsv = Join-Path $ReportsDir ("phase97_pilot_operational_snapshot_" + $Timestamp + ".csv")
$AtlasQueueCsv = Join-Path $ReportsDir ("phase97_pilot_atlas_priority_queue_" + $Timestamp + ".csv")
$ComplianceSummaryCsv = Join-Path $ReportsDir ("phase97_pilot_80h_compliance_summary_" + $Timestamp + ".csv")
$ManagementSnapshotCsv = Join-Path $ReportsDir ("phase97_pilot_management_snapshot_" + $Timestamp + ".csv")
$VerificationDoc = Join-Path $DocsDir "97_ATLAS_80H_REPORTS_ON_PILOT_DATA.md"
...
Set-Content -Path $ReportPath -Value "# RAFTOP CPAP CARE Pro - Phase 97 ATLAS 80h Reports on Pilot Data" -Encoding UTF8
Add-Content -Path $ReportPath -Value "" -Encoding UTF8
Add-Content -Path $ReportPath -Value ("Generated: " + (Get-Date -Format "yyyy-MM-dd HH:mm:ss")) -Encoding UTF8
Add-Content -Path $ReportPath -Value "" -Encoding UTF8
Add-Content -Path $ReportPath -Value "Purpose: verify ATLAS, 80h compliance, and management reports using imported pilot production data." -Encoding UTF8
Add-Content -Path $ReportPath -Value "DATABASE_URL is never printed." -Encoding UTF8
Add-Content -Path $ReportPath -Value "" -Encoding UTF8

Write-Host ""
Write-Host "Running RAFTOP Phase 97 - ATLAS / 80h / Reports Verification on Imported Pilot Data..."
Write-Host ""

Check-ReportStatus "Phase 96 100-row pilot import latest status" "phase96_100_row_controlled_pilot_import_*.md" @(
    "PHASE96_100_ROW_CONTROLLED_PILOT_IMPORT_READY",
    "PHASE96_100_ROW_CONTROLLED_PILOT_IMPORT_READY_WITH_WARNINGS"
)
...
"@
        ...
        if ($SnapshotExit -eq 0) {
            ...
            Add-Result "Pilot operational snapshot exported" "PASS" $SnapshotCsv
        } else {
            Add-Result "Pilot operational snapshot exported" "FAIL" ($SnapshotOutput | Out-String)
        }
    } catch {
        Add-Result "Pilot operational snapshot exported" "FAIL" ("Exception: " + $_.Exception.Message)
    }
}
...
try {
    if (Test-Path $SnapshotCsv) {
        ...
        Add-Result "Pilot operational snapshot readable" "PASS" ("Rows: " + @($Rows).Count)
    }
} catch {
    Add-Result "Pilot operational snapshot readable" "FAIL" ("Could not read snapshot CSV: " + $_.Exception.Message)
}

if (@($Rows).Count -ge 100) {
    Add-Result "Pilot snapshot has 100 rows" "PASS" ("Rows: " + @($Rows).Count)
} else {
    Add-Result "Pilot snapshot has 100 rows" "FAIL" ("Rows: " + @($Rows).Count)
}
...
if (Test-Path $AtlasQueueCsv) {
    Add-Result "ATLAS pilot priority queue CSV created" "PASS" $AtlasQueueCsv
} else {
    Add-Result "ATLAS pilot priority queue CSV created" "FAIL" $AtlasQueueCsv
}

if (@($AtlasQueue).Count -gt 0) {
    Add-Result "ATLAS pilot queue has action rows" "PASS" ("Rows: " + @($AtlasQueue).Count)
} else {
    Add-Result "ATLAS pilot queue has action rows" "WARN" "No action rows. Pilot data may be too clean."
}

if ($Below80h -gt 0) {
    Add-Result "Below 80h patients detected" "PASS" ("Rows: " + $Below80h)
} else {
    Add-Result "Below 80h patients detected" "WARN" "No below-80h rows detected."
}

if ($Compliant80h -gt 0) {
    Add-Result "80h compliant patients detected" "PASS" ("Rows: " + $Compliant80h)
} else {
    Add-Result "80h compliant patients detected" "WARN" "No compliant rows detected."
}

if ($HighAhi -gt 0) {
    Add-Result "High AHI patients detected" "PASS" ("Rows: " + $HighAhi)
} else {
    Add-Result "High AHI patients detected" "WARN" "No high AHI rows detected."
}

if ($HighLeak -gt 0) {
    Add-Result "High leak patients detected" "PASS" ("Rows: " + $HighLeak)
} else {
    Add-Result "High leak patients detected" "WARN" "No high leak rows detected."
}
...
if (Test-Path $ComplianceSummaryCsv) {
    Add-Result "80h pilot compliance summary CSV created" "PASS" $ComplianceSummaryCsv
} else {
    Add-Result "80h pilot compliance summary CSV created" "FAIL" $ComplianceSummaryCsv
}

$ManagementSnapshot = @(
    [PSCustomObject]@{ section = "portfolio"; metric = "pilot_patients"; value = $TotalPatients },
    ...
    [PSCustomObject]@{ section = "readiness"; metric = "ready_for_500_row_stage"; value = "yes_if_buyer_signoff" }
)
...
if (Test-Path $ManagementSnapshotCsv) {
    Add-Result "Pilot management snapshot CSV created" "PASS" $ManagementSnapshotCsv
} else {
    Add-Result "Pilot management snapshot CSV created" "FAIL" $ManagementSnapshotCsv
}

$VerificationDocContent = @"
# RAFTOP CPAP CARE Pro - ATLAS / 80h / Reports Verification on Pilot Data

REQUIRED_MARKER: PHASE97_ATLAS_80H_REPORTS_ON_PILOT_DATA
REQUIRED_MARKER: PILOT_DATA_VERIFIED
REQUIRED_MARKER: ATLAS_QUEUE_GENERATED
REQUIRED_MARKER: EIGHTY_HOURS_COMPLIANCE_VERIFIED
REQUIRED_MARKER: READY_FOR_500_ROW_STAGE_AFTER_SIGNOFF

## Meaning

The imported 100-row pilot data was used to generate operational verification outputs.

## Outputs

Pilot operational snapshot:
$SnapshotCsv
...
Phase 98:
500-row controlled stage plan / apply after buyer signoff.
"@

Set-Content -Path $VerificationDoc -Value $VerificationDocContent -Encoding UTF8

if (Test-Path $VerificationDoc) {
    Add-Result "Phase 97 verification document created" "PASS" $VerificationDoc
} else {
    Add-Result "Phase 97 verification document created" "FAIL" $VerificationDoc
}

foreach ($Marker in @(
    "PHASE97_ATLAS_80H_REPORTS_ON_PILOT_DATA",
    "PILOT_DATA_VERIFIED",
    "ATLAS_QUEUE_GENERATED",
    "EIGHTY_HOURS_COMPLIANCE_VERIFIED",
    "READY_FOR_500_ROW_STAGE_AFTER_SIGNOFF"
)) {
...
if ($script:FailCount -gt 0) {
    $FinalStatus = "PHASE97_ATLAS_80H_REPORTS_ON_PILOT_DATA_FAILED"
    $ExitCode = 1
} elseif ($script:WarnCount -gt 0) {
    $FinalStatus = "PHASE97_ATLAS_80H_REPORTS_ON_PILOT_DATA_READY_WITH_WARNINGS"
    $ExitCode = 0
} else {
    $FinalStatus = "PHASE97_ATLAS_80H_REPORTS_ON_PILOT_DATA_READY"
    $ExitCode = 0
}
...
Write-Host ""
Write-Host "============================================================"
Write-Host "RAFTOP CPAP CARE Pro - Phase 97 ATLAS / 80h / Reports on Pilot Data"
Write-Host "============================================================"
Write-Host ""
