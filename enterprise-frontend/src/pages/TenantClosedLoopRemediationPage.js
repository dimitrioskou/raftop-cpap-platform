...
function getAuditQueueStatus(item) {
  return firstValue(
    item?.auditRemediationStatus,
    item?.audit_remediation_status,
    getRecordRemediationStatus(getVerification(item)),
    'not_started'
  );
}
...
function getRecordRemediationStatus(record) {
  return firstValue(
...
function getRecordRemediationTaskId(record) {
  return firstValue(
...
function getRecordRemediationCreatedAt(record) {
  return firstValue(
    record?.remediationCreatedAt,
    record?.remediation_created_at
  );
}

function getRecordRemediationLastRunAt(record) {
  return firstValue(
    record?.remediationLastRunAt,
    record?.remediation_last_run_at
  );
}

function getRecordRemediationNote(record) {
  return firstValue(
...
function getRecordRemediationPayload(record) {
  return parseJsonObject(
    firstValue(record?.remediationPayload, record?.remediation_payload, {})
  );
}
...
function buildQueuePath({ signalId, taskId, sourceActionId, verdict, remediationStatus, limit = 25 }) {
  ...
  if (remediationStatus) query.set('remediationStatus', remediationStatus);
  query.set('limit', String(limit));

  return `/api/tenant/closed-loop-remediation/queue?${query.toString()}`;
}

function buildRunPath({ signalId, taskId, sourceActionId, verdict, remediationStatus, limit = 25 }) {
  const query = new URLSearchParams();
...
function normalizeQueuePayload(payload) {
  ...
  return {
    ok: safe.ok !== false,
    phase: safe.phase || '19.39-remediation-audit-writeback-ui',
    ...
    auditTaskCreated: firstValue(safe.auditTaskCreated, safe.audit_task_created, 0),
    auditTaskExists: firstValue(safe.auditTaskExists, safe.audit_task_exists, 0),
...
function normalizeRunPayload(payload) {
  ...
  return {
    ...
    writebackUpdated: firstValue(safe.writebackUpdated, safe.writeback_updated, 0),
    writeback_updated: firstValue(safe.writeback_updated, safe.writebackUpdated, 0),
...
function QueueRow({ item, active, onSelect }) {
  ...
  const auditStatus = getAuditQueueStatus(item);
  ...
  return (
    ...
    >
      ...
      <div className="queue-badges">
        ...
        <span className={`mini-badge ${badgeClass(statusTone(auditStatus))}`}>
          Audit: {statusLabel(auditStatus)}
        </span>
        ...
        ) : getRecordRemediationTaskId(record) ? (
          <span className="mini-badge success">
            Audit task: {getRecordRemediationTaskId(record)}
          </span>
        ) : (
...
function AuditWritebackPanel({ record, task }) {
  const auditStatus = getRecordRemediationStatus(record);
  const auditTaskId = getRecordRemediationTaskId(record);
  const auditCreatedAt = getRecordRemediationCreatedAt(record);
  const auditLastRunAt = getRecordRemediationLastRunAt(record);
  const auditNote = getRecordRemediationNote(record);
  const payload = getRecordRemediationPayload(record);
  const taskId = getTaskId(task) || auditTaskId;

  return (
    <section className={`audit-writeback-card ${badgeClass(statusTone(auditStatus))}`}>
      <div className="audit-writeback-header">
        <div>
          <div className="eyebrow">AUDIT WRITEBACK EVIDENCE</div>
          <h2>Remediation writeback</h2>
          <p>
            Απόδειξη ότι το remediation task γράφτηκε πίσω στο closed_loop_verifications audit record.
          </p>
        </div>

        <span className={`badge ${badgeClass(statusTone(auditStatus))}`}>
      ...
      <div className="evidence-grid">
        <DetailRow
          label="Audit remediation status"
          value={statusLabel(auditStatus)}
          tone={statusTone(auditStatus)}
        />
        <DetailRow label="Audit remediation task id" value={auditTaskId} />
        <DetailRow label="Actual task id" value={taskId} />
        <DetailRow label="Remediation created at" value={formatDateTime(auditCreatedAt)} />
        <DetailRow label="Remediation last run at" value={formatDateTime(auditLastRunAt)} />
        <DetailRow label="Payload reason" value={payload.reason} />
        <DetailRow label="Payload source ref" value={payload.source_ref} />
        <DetailRow label="Payload phase" value={payload.phase} />
      </div>
      ...
      {taskId ? (
        <div className="action-row">
          <button
            ...
            onClick={() => openTenantRoute(buildTaskUiPath(taskId))}
          >
            Open Audit Linked Task
          </button>
        </div>
      ) : null}
    </section>
...
function SelectedQueueDetail({ item }) {
  ...
  return (
    <div className="detail-wrap">
      ...
      <AuditWritebackPanel record={record} task={task} />

      <div className="section-subtitle">Verification Evidence</div>
      ...
      {task ? (
        ...
        <div className="empty-state">
          Δεν έχει φορτωθεί πραγματικό remediation task από το queue. Αν το audit task id είναι γεμάτο,
          πάτησε το Open Audit Linked Task πιο πάνω.
        </div>
      )}
    ...
    <section className="run-result-card">
      <div className="run-result-header">
        <div>
          ...
          <h2>Run result</h2>
          <p>
            Αποτέλεσμα τελευταίας εκτέλεσης remediation. Το κρίσιμο metric τώρα είναι και το writebackUpdated.
          </p>
        </div>
        ...
        <div className="run-metrics">
          ...
          <div>
            <span>Writeback Updated</span>
            <strong>{firstValue(result.writebackUpdated, result.writeback_updated, 0)}</strong>
          </div>
        </div>
      </div>
      ...
      <div className="run-list">
        {(result.results || []).slice(0, 8).map((item) => {
          ...
          const verificationRecord = item.verificationRecord || item.verification_record || {};
          const auditStatus = getRecordRemediationStatus(verificationRecord);
          const auditTaskId = getRecordRemediationTaskId(verificationRecord);
          ...
          return (
            <div key={item.verificationId || item.verification_id} className="run-row">
              <div>
                ...
                <span>
                  {item.created ? 'Created' : 'Skipped'} · Task {getTaskId(task) || auditTaskId || '—'}
                </span>
                <span>
                  Audit status: {statusLabel(auditStatus)}
                </span>
              </div>
    ...
    <div className="remediation-page">
      ...
      <section className="hero-card">
        <div>
          ...
          <p>
            Phase 19.39: εδώ βλέπεις failed/warning verifications, δημιουργείς remediation tasks
            και ελέγχεις αν το remediation writeback γράφτηκε πίσω στο audit record.
          </p>
        </div>
        ...
        <div className="hero-metrics">
          ...
          <div>
            ...
            <strong>{queueData?.auditTaskCreated || 0}</strong>
          </div>
          <div>
            <span>Audit exists</span>
            <strong>{queueData?.auditTaskExists || 0}</strong>
          </div>
        </div>
      </section>
      ...
      <section className="toolbar-card">
        ...
        <div className="input-group">
          <label>Audit Remediation Status</label>
          <select
          ...
          >
            ...
            <option value="task_exists">task_exists</option>
        ...
        >
          {running ? 'Running...' : 'Create / Sync Remediation'}
        </button>
      </section>
...
const pageStyles = `
  ...
  .toolbar-card {
    padding: 16px 18px;
    display: grid;
    grid-template-columns: repeat(6, minmax(0, 1fr)) 160px 240px;
    gap: 12px;
    align-items: end;
  }
  ...
  .audit-writeback-card {
    border-color: #fdba74;
    background:
      radial-gradient(circle at top right, rgba(249,115,22,0.10), transparent 32%),
      #ffffff;
  }

  .audit-writeback-card.success {
    border-color: #86efac;
    background:
