...
export default function SystemSecurityExposurePage() {
  ...
  async function runAudit() {
    try {
      ...
      const response = await fetch(`${API_BASE}/api/system/security-exposure-audit`, {
        method: 'GET',
        headers: {
      ...
      setState({
        loading: false,
        error: error.message || 'Security exposure audit failed.',
        payload: null,
        selectedCheck: null
      });
    }
    ...
    >
      <div style={{ maxWidth: 1440, margin: '0 auto' }}>
        ...
        >
          ...
          >
            RAFTOP CPAP CARE Pro / Phase 23.5
          </div>
          ...
          <p style={{ maxWidth: 980, fontSize: 15, opacity: 0.9 }}>
            Exposure audit for routes, secrets, dangerous flags, security headers, CORS, system endpoints and tenant default access.
          </p>
          ...
          >
            ...
            <button
              onClick={() => {
                window.location.href = '/system/database-backup-safety';
              }}
              style={headerButton}
            >
              Backup Safety
        ...
        {state.error && (
          <div style={errorBox}>
            <strong>Security Exposure Error</strong>
        ...
        {state.loading && (
          <div style={loadingBox}>
            Running security exposure audit...
          </div>
        )}
        ...
        {!state.loading && state.payload && (
          <>
            ...
            >
              ...
              <MetricCard label="Route Warnings" value={summary.routeWarnings} tone={summary.routeWarnings > 0 ? 'warning' : 'success'} />
              <MetricCard label="Route Failures" value={summary.routeFailures} tone={summary.routeFailures > 0 ? 'danger' : 'success'} />
              <MetricCard label="Secret Warnings" value={summary.secretWarnings} tone={summary.secretWarnings > 0 ? 'warning' : 'success'} />
              <MetricCard label="Header Failures" value={summary.headerFailures} tone={summary.headerFailures > 0 ? 'danger' : 'success'} />
            </section>

            <section style={panelStyleWithMargin}>
              <h2 style={{ margin: 0, fontSize: 24 }}>Security Verdict</h2>
              ...
              >
                <strong>
                  {readiness === 'READY'
                    ? 'Security exposure gate passed.'
                    : readiness === 'NEEDS_ATTENTION'
                      ? 'No hard blocker, but security exposure warnings remain.'
                      : 'Security exposure is blocked. Fix critical failures first.'}
                </strong>
            ...
            <section style={panelStyleWithMargin}>
              ...
              >
                <div>
                  <h2 style={{ margin: 0, fontSize: 24 }}>Exposure Checks</h2>
