...
function transitionLabel(transition = '') {
  if (transition === 'start') return 'Start';
  if (transition === 'escalate') return 'Escalate';
  if (transition === 'resolve') return 'Resolve';
  if (transition === 'reopen') return 'Reopen';
...
function normalizeTransitionEvidence(payload, task, transition) {
  ...
  return {
    ...
    previousStatus: firstValue(safePayload.previousStatus, safePayload.previous_status, '—'),
    taskStatus,
    signalId,
    followupStatus,
  ...
  return (
    <section className={`transition-evidence-card ${tone}`}>
      <div className="transition-evidence-header">
        <div>
          <div className="eyebrow evidence-eyebrow">TASK TRANSITION WRITEBACK EVIDENCE</div>
          <h2>Signal context updated</h2>
          <p>
            Απόδειξη ότι η αλλαγή status του task συγχρονίστηκε πίσω στο linked ATLAS signal.
          </p>
        </div>
      ...
      const payload = await apiRequest(
        `/api/tenant/tasks-unified/${encodeURIComponent(taskId)}/${transition}`,
        {
          ...
          body: {
            ...
            phase: '19.25-surface-transition-writeback-ui'
          },
    ...
    <div className="tenant-tasks-page">
      ...
      <section className="hero-card">
        <div>
          ...
          <h1>Tenant Tasks</h1>
          <p>
            Ενιαία προβολή των ATLAS-generated follow-up tasks. Το Phase 19.25 δείχνει
            καθαρά αν το status transition γράφτηκε πίσω στο linked ATLAS signal.
          </p>
