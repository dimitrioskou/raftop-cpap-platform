...
export default function SystemFrontendConfigAuditPage() {
  ...
  async function runAudit() {
    try {
      ...
      const payload = await runFrontendProductionConfigAudit();
      ...
      setState({
        loading: false,
        error: error.message || 'Frontend production config audit failed.',
        payload: null,
        selectedCheck: null
      });
    }
    ...
    >
      <div style={{ maxWidth: 1440, margin: '0 auto' }}>
        ...
        >
          ...
          >
            RAFTOP CPAP CARE Pro / Phase 23.3
          </div>

          <h1 style={{ margin: '14px 0 8px', fontSize: 38, lineHeight: 1.1 }}>
            Frontend Production Config Audit
          </h1>

          <p style={{ maxWidth: 980, fontSize: 15, opacity: 0.9 }}>
            Frontend hardening gate for API URL, production build mode, HTTPS, tenant context, dev controls, public env exposure and backend reachability.
          </p>
          ...
          >
            ...
            <button
              onClick={() => {
                window.location.href = '/system/backend-config';
              }}
              style={headerButton}
            >
              Backend Config
        ...
        {state.error && (
          <div style={errorBox}>
            <strong>Frontend Config Audit Error</strong>
        ...
        {state.loading && (
          <div style={loadingBox}>
            Running frontend production config audit...
          </div>
        )}
        ...
        {!state.loading && state.payload && (
          <>
            ...
            >
              ...
              <MetricCard label="API Failures" value={summary.apiFailures} tone={summary.apiFailures > 0 ? 'danger' : 'success'} />
            </section>

            <section style={panelStyleWithMargin}>
              <h2 style={{ margin: 0, fontSize: 24 }}>Frontend Config Verdict</h2>
              ...
              >
                <strong>
                  {readiness === 'READY'
                    ? 'Frontend production config gate passed.'
                    : readiness === 'NEEDS_ATTENTION'
                      ? 'No hard blocker, but frontend production warnings remain.'
                      : 'Frontend production config is blocked. Fix critical failures first.'}
                </strong>
            ...
            <section style={panelStyleWithMargin}>
              <h2 style={{ margin: 0, fontSize: 24 }}>Runtime Snapshot</h2>
              ...
              >
                <div><strong>Frontend URL:</strong> {state.payload.runtime?.location?.href || '-'}</div>
                <div><strong>Frontend hostname:</strong> {state.payload.runtime?.location?.hostname || '-'}</div>
                <div><strong>Frontend protocol:</strong> {state.payload.runtime?.location?.protocol || '-'}</div>
                <div><strong>API base:</strong> {state.payload.runtime?.apiBase || '-'}</div>
                <div><strong>NODE_ENV:</strong> {state.payload.runtime?.nodeEnv || '-'}</div>
                <div><strong>Local frontend:</strong> {String(state.payload.runtime?.localFrontend)}</div>
                <div><strong>Local API:</strong> {String(state.payload.runtime?.localApi)}</div>
                <div><strong>Likely production runtime:</strong> {String(state.payload.runtime?.likelyProductionRuntime)}</div>
              </div>
            </section>
            ...
            <section style={panelStyleWithMargin}>
              ...
              >
                <div>
                  <h2 style={{ margin: 0, fontSize: 24 }}>Frontend Checks</h2>
