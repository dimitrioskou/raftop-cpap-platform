...
export default function SystemDatabaseBackupSafetyPage() {
  ...
  async function runAudit() {
    try {
      ...
      const response = await fetch(`${API_BASE}/api/system/database-backup-safety-audit`, {
        method: 'GET',
        headers: {
      ...
      setState({
        loading: false,
        error: error.message || 'Database backup safety audit failed.',
        payload: null,
        selectedCheck: null
      });
    }
    ...
    >
      <div style={{ maxWidth: 1440, margin: '0 auto' }}>
        ...
        >
          ...
          >
            RAFTOP CPAP CARE Pro / Phase 23.4
          </div>

          <h1 style={{ margin: '14px 0 8px', fontSize: 38, lineHeight: 1.1 }}>
            Database Backup / Restore Safety Check
          </h1>

          <p style={{ maxWidth: 980, fontSize: 15, opacity: 0.9 }}>
            Release safety gate for database connectivity, core tables, backup tooling, backup directory, backup policy and restore-lock exposure.
          </p>
          ...
          >
            ...
            <button
              onClick={() => {
                window.location.href = '/system/frontend-config';
              }}
              style={headerButton}
            >
              Frontend Config
        ...
        {state.error && (
          <div style={errorBox}>
            <strong>Database Backup Safety Error</strong>
        ...
        {state.loading && (
          <div style={loadingBox}>
            Running database backup safety audit...
          </div>
        )}
        ...
        {!state.loading && state.payload && (
          <>
            ...
            >
              ...
              <MetricCard label="Backup Warnings" value={summary.backupWarnings} tone={summary.backupWarnings > 0 ? 'warning' : 'success'} />
              <MetricCard label="Restore Warnings" value={summary.restoreWarnings} tone={summary.restoreWarnings > 0 ? 'warning' : 'success'} />
            </section>

            <section style={panelStyleWithMargin}>
              <h2 style={{ margin: 0, fontSize: 24 }}>Backup / Restore Verdict</h2>
              ...
              >
                <strong>
                  {readiness === 'READY'
                    ? 'Database backup / restore safety gate passed.'
                    : readiness === 'NEEDS_ATTENTION'
                      ? 'No hard blocker, but backup/restore warnings remain.'
                      : 'Database backup safety is blocked. Fix critical failures first.'}
                </strong>
            ...
            <section style={panelStyleWithMargin}>
              ...
              >
                <div>
                  <h2 style={{ margin: 0, fontSize: 24 }}>Backup Safety Checks</h2>
