  ...
  items: [
    ...
    },
    {
      ...
      badges: [
        ...
        { label: 'TASK CREATED', tone: 'success' }
      ]
    }
  ],
  debug: {
...
function getAtlasCategory(item) {
  return firstValue(item?.atlasCategory, item?.atlas_category, item?.category, '—');
}
...
function buildPatientTasksLink(item) {
  const email = getPatientEmail(item) || 'patient@raftop.local';
  ...
  if (!linkedTaskId) {
    if (item?.priority === 'critical') query.set('status', 'escalated');
    else if (item?.priority === 'warning') query.set('status', 'pending');
  }
...
function calculateWritebackSummary(items = []) {
  ...
  for (const item of items) {
    ...
    if (state.linkedSignalId || state.coachingContextId) {
    ...
    try {
      ...
      setFlashMessage(
        evidence.status && evidence.status !== 'unknown'
          ? `Το task δημιουργήθηκε. Writeback status: ${writebackLabel(evidence.status)}.`
          : 'Το task δημιουργήθηκε επιτυχώς από το ATLAS Action Center.'
      );
      ...
      <section className="hero-card">
        <div>
          ...
          <p>
            ...
            Το Phase 19 εμφανίζει πλέον καθαρά αν κάθε action συγχρονίστηκε πίσω σε linked signal ή coaching context.
          </p>
        </div>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="action-meta">
                      {item.subtitle || getPatientEmail(item) || '—'}
                    </div>
          ...
          {selectedItem ? (
            <div className="detail-wrap">
              ...
              <div className="detail-row">
                ...
                <span>{getPatientEmail(selectedItem) || '—'}</span>
              </div>
              ...
              <div className="detail-actions">
                {canCreateTask(selectedItem) ? (
                  ...
                  <button type="button" className="success-btn" disabled>
                    Task Already Linked
                  </button>
                )}
