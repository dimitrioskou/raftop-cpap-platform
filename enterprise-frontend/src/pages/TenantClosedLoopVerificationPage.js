...
function buildRemediationUiPath({ taskId, signalId, sourceActionId, verdict }) {
  const query = new URLSearchParams();
  ...
  const qs = query.toString();
  return qs ? `/tenant/closed-loop-remediation?${qs}` : '/tenant/closed-loop-remediation';
}
...
function normalizeBackendPayload(payload) {
  ...
  return {
    ok: safe.ok !== false,
    phase: safe.phase || '19.37-closed-loop-remediation-shortcut',
  ...
  return (
    <section className={`kpi-dashboard-card ${badgeClass(healthTone)}`}>
      ...
      <div className="kpi-actions">
        ...
        <button
          type="button"
          className="primary-remediation-btn"
                  ...
                  <div className="problem-actions">
                    ...
                    <button
                      ...
                      onClick={() =>
                        item.signal_id
                          ? openTenantRoute(
                              buildRemediationUiPath({
                                signalId: item.signal_id,
                                verdict: item.failed > 0 ? 'failed' : 'warning'
                              })
                            )
                          : null
                      }
                ...
                <div key={getRecordId(record)} className="problem-row">
                  ...
                  <button
                    ...
                    onClick={() =>
                      openTenantRoute(
                        buildRemediationUiPath({
                          taskId: getRecordTaskId(record),
                          signalId: getRecordSignalId(record),
                          sourceActionId: getRecordSourceActionId(record),
                          verdict: getRecordVerdict(record)
                        })
                      )
                    }
                  >
...
function ServerVerdictPanel({ result, task, signal, taskId, signalId, sourceActionId }) {
  ...
  const remediationVerdict =
    verdict && ['failed', 'warning'].includes(String(verdict).toLowerCase())
      ? verdict
      : '';
          ...
          onClick={() =>
            openTenantRoute(
              buildRemediationUiPath({
                ...
                sourceActionId: getRecordSourceActionId(record),
                verdict:
                  ['failed', 'warning'].includes(String(verdict).toLowerCase())
                    ? verdict
                    : ''
              })
            )
    ...
    <div className="closed-loop-page">
      ...
      <section className="hero-card">
        <div>
          ...
          <p>
            Phase 19.37: η σελίδα δείχνει operational KPI dashboard και έχει άμεση μετάβαση
            από failed/warning verification στο Remediation Center.
          </p>
        </div>
              ...
              onClick={() =>
                openTenantRoute(
                  buildRemediationUiPath({
                    ...
                    verdict:
                      result?.verdict &&
                      ['failed', 'warning'].includes(String(result.verdict).toLowerCase())
                        ? result.verdict
                        : ''
                  })
                )
...
const pageStyles = `
  ...
  .primary-remediation-btn:disabled,
  .ghost-btn:disabled {
    opacity: 0.65;
    cursor: not-allowed;
  }
