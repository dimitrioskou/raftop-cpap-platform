...
export default function SystemProductionReadinessPage() {
  ...
  async function runAudit() {
    try {
      ...
      const dangerousDevControls = isDangerousDevControlsEnabled();
      ...
      const response = await fetch(
        `${API_BASE}/api/system/production-readiness-audit?tenantId=${encodeURIComponent(cleanTenantId)}`,
        {
          method: 'GET',
          headers: {
            ...
            'x-frontend-dangerous-dev-controls': dangerousDevControls ? 'true' : 'false'
          }
        }
      );
      ...
      setState({
        loading: false,
        error: error.message || 'Production readiness audit failed.',
        payload: null,
        selectedCheck: null
      });
    }
    ...
    >
      <div style={{ maxWidth: 1440, margin: '0 auto' }}>
        ...
        >
          ...
          >
            RAFTOP CPAP CARE Pro / Phase 23.1
          </div>
          ...
          <p style={{ maxWidth: 980, fontSize: 15, opacity: 0.9 }}>
            Release-candidate gate for backend, database, SaaS stability, route stability, tenant metadata, frontend safety and monitoring.
          </p>
          ...
          >
            ...
            <button
              onClick={() => {
                window.location.href = '/system/stability';
              }}
              style={headerButton}
            >
              Route Stability
            ...
            <button
              onClick={() => {
                window.location.href = '/system/monitoring';
              }}
              style={headerButton}
            >
              Monitoring
        ...
        {state.error && (
          <div style={errorBox}>
            <strong>Production Readiness Error</strong>
        ...
        {!state.loading && state.payload && (
          <>
            ...
            >
              ...
              <MetricCard label="Config Failures" value={summary.configFailures} tone={summary.configFailures > 0 ? 'danger' : 'success'} />
              <MetricCard label="Backend Failures" value={summary.backendFailures} tone={summary.backendFailures > 0 ? 'danger' : 'success'} />
              <MetricCard label="Frontend Failures" value={summary.frontendFailures} tone={summary.frontendFailures > 0 ? 'danger' : 'success'} />
              <MetricCard label="Data Failures" value={summary.dataFailures} tone={summary.dataFailures > 0 ? 'danger' : 'success'} />
            </section>

            <section style={panelStyleWithMargin}>
              <h2 style={{ margin: 0, fontSize: 24 }}>Release Verdict</h2>
                ...
                <strong>
                  {readiness === 'READY'
                    ...
                    : readiness === 'NEEDS_ATTENTION'
                      ...
                      : 'Release candidate is blocked. Fix critical failures first.'}
                </strong>
            ...
            <section style={panelStyleWithMargin}>
              ...
              >
                <div>
                  <h2 style={{ margin: 0, fontSize: 24 }}>Readiness Checks</h2>
