...
export default function SystemTenantCleanupPage() {
  ...
  async function runAudit() {
    try {
      ...
      const response = await fetch(`${API_BASE}/api/system/tenant-cleanup-audit`, {
        method: 'GET',
        headers: {
      ...
      setState({
        loading: false,
        error: error.message || 'Tenant cleanup audit failed.',
        payload: null,
        selectedCheck: null
      });
    }
    ...
    >
      <div style={{ maxWidth: 1440, margin: '0 auto' }}>
        ...
        >
          ...
          >
            RAFTOP CPAP CARE Pro / Phase 23.6
          </div>

          <h1 style={{ margin: '14px 0 8px', fontSize: 38, lineHeight: 1.1 }}>
            Demo Tenant / Raftopoulos Cleanup Audit
          </h1>

          <p style={{ maxWidth: 980, fontSize: 15, opacity: 0.9 }}>
            Readiness gate for tenant subscriptions, tenant profiles, duplicates, demo/test noise, Raftopoulos metadata and sales-demo cleanliness.
          </p>
        ...
        {state.error && (
          <div style={errorBox}>
            <strong>Tenant Cleanup Error</strong>
        ...
        {!state.loading && state.payload && (
          <>
            ...
            >
              ...
              <MetricCard label="Tenant Failures" value={summary.tenantFailures} tone={summary.tenantFailures > 0 ? 'danger' : 'success'} />
              <MetricCard label="Alignment Failures" value={summary.alignmentFailures} tone={summary.alignmentFailures > 0 ? 'danger' : 'success'} />
              <MetricCard label="Demo Warnings" value={summary.demoWarnings} tone={summary.demoWarnings > 0 ? 'warning' : 'success'} />
            </section>

            <section style={panelStyleWithMargin}>
              <h2 style={{ margin: 0, fontSize: 24 }}>Tenant Cleanup Verdict</h2>
              ...
              >
                <strong>
                  {readiness === 'READY'
                    ? 'Tenant cleanup gate passed.'
                    : readiness === 'NEEDS_ATTENTION'
                      ? 'No hard blocker, but tenant/demo cleanup warnings remain.'
                      : 'Tenant cleanup is blocked. Fix critical failures first.'}
                </strong>
            ...
            <section style={panelStyleWithMargin}>
              ...
              >
                <div>
                  <h2 style={{ margin: 0, fontSize: 24 }}>Cleanup Checks</h2>
