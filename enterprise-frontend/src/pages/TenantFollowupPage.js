...
function scoreArrayCandidate(arr) {
  ...
  for (const item of arr.slice(0, 5)) {
    ...
    if (keys.includes('outcome')) score += 2;
    if (keys.includes('due_at') || keys.includes('dueat') || keys.includes('scheduled_at')) score += 2;
...
function normalizeRow(item, index) {
  return {
    id: String(item.id || item.followup_id || item.followupId || `FU-${index + 1}`),
    ...
    priority: String(item.priority || 'warning').toLowerCase().includes('critical') ? 'critical' : 'warning',
    outcome: item.outcome || item.result || 'Pending',
    dueAt: item.due_at || item.dueAt || item.scheduled_at || item.scheduledAt || null,
...
function getKind(value) {
  ...
  if (raw.includes('critical') || raw.includes('overdue')) return 'danger';
  if (raw.includes('warning') || raw.includes('pending') || raw.includes('open')) return 'warning';
  return 'success';
}
...
export default function TenantFollowupPage() {
  ...
  const loadRows = useCallback(async (signal) => {
    ...
    try {
      const result = await fetchFirstUsablePayload(
        ['/api/tenant/followup', '/api/followup', '/api/tenant/atlas/queue'],
        signal
      );
      ...
      setPayloadDebug('Request failed before usable follow-up payload was found.');
    } finally {
  ...
  const stats = useMemo(() => ({
    ...
    resolved: rows.filter((r) => String(r.status).toLowerCase() === 'resolved').length
  }), [rows]);
  ...
  return (
    <div style={{ padding: 20 }}>
      ...
      >
        <div>
          ...
          <div style={{ color: '#cbd5e1', marginTop: 6 }}>
            Premium follow-up workflow workspace.
          </div>
        </div>
      ...
      >
        ...
        <MetricCard label="Critical" value={stats.critical} tone="dark" />
      ...
      <div style={{ ...toolbarCardStyle(), marginBottom: 16 }}>
        <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr auto', gap: 12 }}>
          <input
            ...
            placeholder="Search by patient, owner, outcome..."
            style={{
              width: '100%',
      ...
      <div style={tableContainerStyle()}>
        ...
        {loading ? (
          <PageStateCard
            ...
            message="Fetching follow-up data from the active tenant endpoints."
          />
            ...
            <table style={{ width: '100%', borderCollapse: 'separate', borderSpacing: 0, minWidth: 980 }}>
              <thead>
                <tr style={{ textAlign: 'left' }}>
                  {['Patient', 'Owner', 'Priority', 'Outcome', 'Due', 'Status'].map((heading) => (
                    <th
                      key={heading}
                      style={{
                        padding: '12px 10px',
              ...
              <tbody>
                {filteredRows.map((row) => (
                  <tr key={row.id}>
                    ...
                    <td style={{ padding: '16px 10px', borderBottom: '1px solid #f2f4f7', verticalAlign: 'top' }}>
                      <span style={statusBadgeStyle(getKind(row.priority))}>{row.priority}</span>
                    </td>
                    <td style={{ padding: '16px 10px', borderBottom: '1px solid #f2f4f7', verticalAlign: 'top' }}>
                      {row.outcome}
                    </td>
