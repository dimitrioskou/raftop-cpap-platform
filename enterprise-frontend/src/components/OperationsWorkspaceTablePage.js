...
function scoreGenericArrayCandidate(arr) {
  ...
  for (const item of arr.slice(0, 5)) {
    ...
    if (
      ...
      keys.includes('doctor_name') ||
      keys.includes('doctorname')
    ) {
      score += 2;
    }
    if (
...
function findTotalCountInPayload(payload) {
...
function extractRows(payload, responseKeys = []) {
  ...
  const totalCount = findTotalCountInPayload(payload);

  if (payload?.ok === true && totalCount === 0) {
    return {
      rows: [],
      debug: `API returned valid empty dataset. Shape: ${describePayloadShape(payload)}`,
      validEmpty: true
    };
  }
...
export default function OperationsWorkspaceTablePage({
  title,
  subtitle,
  entityLabel,
  endpointGroups,
      ...
      <div style={tableContainerStyle()}>
        ...
        {loading ? (
          ...
          <PageStateCard
            ...
            message="There are no records available right now."
            actionLabel="Refresh"
