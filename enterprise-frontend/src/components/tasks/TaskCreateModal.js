...
export default function TaskCreateModal({ open, onClose, onCreated }) {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const response = await apiPost('/api/tenant/tasks', payload);

      setSuccess('Task created successfully.');

      if (typeof onCreated === 'function') {
        onCreated(response?.task || null);
      }
      ...
      <div style={cardStyle()}>
        ...
        >
          <div>
            ...
            <div style={{ color: '#667085', marginTop: 6 }}>
              Create a task and add it to the execution workflow.
            </div>
          </div>
        ...
        <form onSubmit={handleSubmit}>
          ...
          >
            ...
            <div>
              <div style={labelStyle()}>Follow-up ID</div>
              <input
                value={form.followup_id}
                onChange={(e) => updateField('followup_id', e.target.value)}
                style={inputStyle()}
                placeholder="Optional follow-up ID"
              />
            </div>
            ...
            <div>
              <div style={labelStyle()}>Due at</div>
              <input
                type="datetime-local"
                value={form.due_at}
