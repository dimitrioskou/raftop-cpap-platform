...
export default function RuntimeAclGuard({
  children,
  feature = null,
  fallbackTitle = 'Access unavailable'
}) {
  ...
  const routeAccess = canAccessRoute({
    runtime,
    role,
  ...
  const allowed = routeAccess.allowed && featureAllowed;
  ...
  return (
    <main style={page}>
      <section style={card}>
        ...
        <h1 style={title}>{fallbackTitle}</h1>

        <p style={text}>
          This page or feature is not available for the current role, tenant, plan,
          module configuration, or entitlement state.
        </p>
        ...
        <div style={grid}>
          ...
          <Info label="Reason" value={routeAccess.reason || 'Feature entitlement blocked'} />
        </div>
